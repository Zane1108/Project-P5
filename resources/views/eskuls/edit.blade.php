                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('eskuls.update', $eskuls->id) }}" method="POST"
                            ...
                            @csrf
                            <div class="mb-3">
                                <label class="form-label">Ekstakulikuler</label>
                                <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                    name="eskul" value="{{ $eskuls->eskul}}" placeholder="Eskuls Name" required>
                                @error('nama')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" class="form-control @error('deskirpsi') is-invalid @enderror" name="deskripsi"
                                    rows="3" placeholder="deskirpsi" required>{{ $eskuls->deskripsi }}</textarea>
                                @error('deskirpsi')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control @error('image') is-invalid @enderror"
                                    name="image" value="{{ $eskuls->image }}"></input>
                                @error('image')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
